...
export class AnalysesCoreApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param analysisId Analysis ID
     */
    public async getAnalysisBasicInfo_1(analysisId: number, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'analysisId' is not null or undefined
        if (analysisId === null || analysisId === undefined) {
            throw new RequiredError("AnalysesCoreApi", "getAnalysisBasicInfo_1", "analysisId");
        }
...
export class AnalysesCoreApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to getAnalysisBasicInfo_1
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getAnalysisBasicInfo_1WithHttpInfo(response: ResponseContext): Promise<HttpInfo<AnalysisBasicInfoOutputBody >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
